...
import { StyledContent, AddConnectionButton } from '../../styles/StyledServiceConnections';
...
function ServiceConnectionModal(props) {
  ...
  const initializeConsentUrl = () => {
    var route = buildConsentUrl('365OFF', serviceConnectionInputs['tenantDomain']);
    window.location.href = route;
  };

  return (
    <Modal onClose={() => setOpen(false)} onOpen={() => setOpen(true)} open={open} trigger={<Button>Modal</Button>}>
      <Header icon="id card outline" content="Add a new Service Connection"></Header>
      <Modal.Content>
        <Modal.Description>
          <StyledContent>
            By clicking proceed you will be redirect to {props.vendor} where you will be asked to grant access for this
            application to {props.name}. This authorization will need to be approved by an administrator.
            <br />
            <br />
